...
interface AboutPropertyDetails {
  zoneType: string;
  locationHub: string;
...
export default function AboutProperty() {
  const [formData, setFormData] = useState<AboutPropertyDetails>({
    zoneType: '',
    locationHub: '',
    otherLocationHub: '',
    propertyCondition: '',
  ...
  const handleSingleSelect = (field: keyof AboutPropertyDetails, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
      ...(field === 'locationHub' && value !== 'Others' && { otherLocationHub: '' }),
      ...(field === 'propertyCondition' && value !== 'Bare shell' && { 
        constructionStatus: '',
        flooring: ''
      }),
    }));
  ...
  const SingleSelectGroup = ({ 
    ...
    label: string; 
    field: keyof AboutPropertyDetails; 
    options: string[];
  ...
  const MeasurementInput = ({ 
    ...
    label: string; 
    name: keyof AboutPropertyDetails;
    unit: string;
    icon: React.ElementType;
  }) => (
    <div>
  ...
  return (
    <div className="w-full max-w-md pl-4 sm:pl-6 md:pl-8">
      ...
      <div className="space-y-6">
        ...
        {formData.propertyCondition !== 'Bare shell' && (
          <MeasurementInput
            label="Carpet Area"
            name="carpetArea"
            unit="sq. ft."
