...
//     console.log("Fetching Lease Retail Store details for:", propertyId);
//     setLoading(true);

//     try {
//      // In your useEffect fetch function
// const response = await axios.get(`/api/commercial/lease/retailstore/${propertyId}?t=${Date.now()}`, {
//   headers: {
...
//         const retailshop = response.data.data;
//         console.log("Fetched data:", retailshop);

//         //  Transform leaseTerms structure for frontend
//         const frontendLeaseTerms = {
//           leaseAmount: retailshop.leaseTerms?.leaseDetails?.leaseAmount || {
//             amount: 0,
//             type: "fixed",
//             duration: 0,
//             durationUnit: "years",
//           },
//           leaseTenure: {
//             minimumTenure: Number(retailshop.leaseTerms?.tenureDetails?.minimumTenure) || 0,
//             minimumUnit: retailshop.leaseTerms?.tenureDetails?.minimumUnit || "years",
//             maximumTenure: Number(retailshop.leaseTerms?.tenureDetails?.maximumTenure) || 0,
//             maximumUnit: retailshop.leaseTerms?.tenureDetails?.maximumUnit || "years",
//             lockInPeriod: Number(retailshop.leaseTerms?.tenureDetails?.lockInPeriod) || 0,
//             lockInUnit: retailshop.leaseTerms?.tenureDetails?.lockInUnit || "years",
//             noticePeriod: Number(retailshop.leaseTerms?.tenureDetails?.noticePeriod) || 0,
//             noticePeriodUnit: retailshop.leaseTerms?.tenureDetails?.noticePeriodUnit || "months",
//           },
//           maintenanceAmount: retailshop.leaseTerms?.maintenanceAmount || { amount: 0, frequency: "monthly" },
//           otherCharges: retailshop.leaseTerms?.otherCharges || {},
//           brokerage: retailshop.leaseTerms?.brokerage || { required: "no", amount: 0 },
//           availability: retailshop.leaseTerms?.availability || {},
//         };

//         //  Set formData
//         setFormData((prev) => ({
//           ...prev,
//           propertyId: retailshop.propertyId,
//           basicInformation: retailshop.basicInformation || prev.basicInformation,
//           propertyDetails: retailshop.propertyDetails || prev.propertyDetails,
//           leaseTerms: frontendLeaseTerms,
//           contactInformation: retailshop.contactInformation || prev.contactInformation,
//           media: retailshop.media || prev.media,
//         }));

//         toast.success("Loaded property details for editing.");
//       } else {
...
// //       ? `/api/commercial/lease/retail-store/${propertyId}`
...
// //       toast.success(
// //         propertyId
// //           ? 'Retail store lease listing updated successfully!'
// //           : 'Retail store lease listing created successfully!'
// //       );
// //       navigate('/Userdashboard/properties'); // redirect to list/dashboard after save
// //     } 
...
//     const convertMediaWithHandling = async () => {
//       try {
//         const photos: any = {};
        
//         for (const [category, files] of Object.entries(formData.media.photos)) {
//           if (files && files.length > 0) {
//             photos[category] = await Promise.all(
...
//         const videoTour = formData.media.videoTour 
//           ? await convertFileToBase64(formData.media.videoTour)
//           : null;

//         const documents = formData.media.documents.length > 0
//           ? await Promise.all(
...
//         return { photos, videoTour, documents };
//       } catch (error) {
//         console.error('Error in media conversion:', error);
//         throw new Error('Media conversion failed');
//       }
//     };

//     const convertedMedia = await convertMediaWithHandling();

//     // 🔥 CORRECTED DATA STRUCTURE - Match backend schema exactly
//     const transformedData = {
//       propertyId: formData.propertyId, // Include propertyId for updates
//       basicInformation: {
//         title: formData.basicInformation.title || '',
...
//       // 🔥 retailStoreDetails at root level (as per your schema)
//       retailStoreDetails: {
//         location: formData.propertyDetails.retailStoreDetails.location || '',
//         anchorStores: formData.propertyDetails.retailStoreDetails.anchorStores || false,
//         footfallData: formData.propertyDetails.retailStoreDetails.footfallData || '',
//         signageAllowed: formData.propertyDetails.retailStoreDetails.signageAllowed || false,
//         sharedWashrooms: formData.propertyDetails.retailStoreDetails.sharedWashrooms || false,
//         fireExit: formData.propertyDetails.retailStoreDetails.fireExit || false
//       },
      
//       // 🔥 propertyDetails without retailStoreDetails
//       propertyDetails: {
//         area: {
...
//       // 🔥 CORRECTED leaseTerms structure
//       leaseTerms: {
//         leaseDetails: {
//           leaseAmount: {
//             amount: Number(formData.leaseTerms.leaseAmount.amount) || 0,
//             type: formData.leaseTerms.leaseAmount.type || 'fixed',
//             duration: Number(formData.leaseTerms.leaseAmount.duration) || 0,
//             durationUnit: formData.leaseTerms.leaseAmount.durationUnit || 'years'
//           }
//         },
//         tenureDetails: {
//           minimumTenure: Number(formData.leaseTerms.leaseTenure.minimumTenure) || 0,
//           minimumUnit: formData.leaseTerms.leaseTenure.minimumUnit || 'years',
//           maximumTenure: Number(formData.leaseTerms.leaseTenure.maximumTenure) || 0,
//           maximumUnit: formData.leaseTerms.leaseTenure.maximumUnit || 'years',
//           lockInPeriod: Number(formData.leaseTerms.leaseTenure.lockInPeriod) || 0,
//           lockInUnit: formData.leaseTerms.leaseTenure.lockInUnit || 'years',
//           noticePeriod: Number(formData.leaseTerms.leaseTenure.noticePeriod) || 0,
//           noticePeriodUnit: formData.leaseTerms.leaseTenure.noticePeriodUnit || 'months'
//         },
//         maintenanceAmount: {
...
//         otherCharges: {
//           water: {
//             amount: Number(formData.leaseTerms.otherCharges.water?.amount) || 0,
//             type: formData.leaseTerms.otherCharges.water?.type || 'inclusive'
//           },
//           electricity: {
//             amount: Number(formData.leaseTerms.otherCharges.electricity?.amount) || 0,
//             type: formData.leaseTerms.otherCharges.electricity?.type || 'inclusive'
//           },
//           gas: {
//             amount: Number(formData.leaseTerms.otherCharges.gas?.amount) || 0,
//             type: formData.leaseTerms.otherCharges.gas?.type || 'inclusive'
//           },
//           others: {
//             amount: Number(formData.leaseTerms.otherCharges.others?.amount) || 0,
//             type: formData.leaseTerms.otherCharges.others?.type || 'inclusive'
//           }
//         },
//         brokerage: {
//           required: formData.leaseTerms.brokerage.required || 'no',
//           amount: Number(formData.leaseTerms.brokerage.amount) || 0
//         },
//         availability: {
//           date: formData.leaseTerms.availability.date || new Date(),
//           availableImmediately: formData.leaseTerms.availability.availableImmediately || false,
//           preferredSaleDuration: formData.leaseTerms.availability.preferredSaleDuration || '',
//           noticePeriod: formData.leaseTerms.availability.noticePeriod || '',
//           isPetsAllowed: formData.leaseTerms.availability.isPetsAllowed || false,
...
//     const url = propertyId
//       ? `/api/commercial/lease/retailstore/${propertyId}`
//       : `/api/commercial/lease/retailstore`;
...
//     } else {
//       toast.error("Failed to save retail store lease listing. Please try again.");
//     }
//   } finally {
//     setIsSubmitting(false);
//   }
...
useEffect(() => {
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`/api/commercial/lease/retailstore/${propertyId}`);
      if (response.data?.success) {
        const data = response.data.data;
        
        setFormData(prev => ({
          ...prev,
          propertyId: data.propertyId || data._id, // Use propertyId if available, fallback to _id
          basicInformation: {
            ...prev.basicInformation,
            ...data.basicInformation // Populate all basicInformation fields
          },
          propertyDetails: {
            ...prev.propertyDetails,
            ...data.propertyDetails // Populate all propertyDetails fields
          },
          leaseTerms: {
            ...prev.leaseTerms,
            ...data.leaseTerms // Populate all leaseTerms fields
          },
          contactInformation: {
            ...prev.contactInformation,
            ...data.contactInformation // Populate all contactInformation fields
          },
          media: {
            ...prev.media,
            ...data.media // Populate all media fields
          }
        }));
