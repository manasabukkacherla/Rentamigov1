...
const services: Service[] = [
  ...
  },
  {
    ...
    shortDescription: "Revenue records transfer from previous owner to current owner",
    description: [
      "Through Sale Deed",
  ...
  },
  {
    ...
    shortDescription: "Name change on BESCOM Electricity Bill from previous owner to current owner",
    description: [
  ...
  },
  {
    ...
    shortDescription: "Name change on Tax receipt from previous owner to current owner",
    description: [
      "Ownership Proof",
  ...
  },
  {
    ...
    shortDescription: "Determining market value of property by certified experts",
    description: [
...
export default function ServicesPage() {
  ...
  const handleSendOtp = async () => {
    try {
      ...
      const response = await axios.post(
        "https://c5zaskxsitwlc33abxxgi3smli0lydfl.lambda-url.us-east-1.on.aws/api/verify/start",
        {
  ...
  const handleVerifyOtp = async () => {
    try {
      const formattedPhoneNumber = `+91${phoneNumber}`; // Adding +91 prefix
      const response = await axios.post(
        "https://c5zaskxsitwlc33abxxgi3smli0lydfl.lambda-url.us-east-1.on.aws/api/verify/check",
        {
          to: formattedPhoneNumber,
          code: otp,
        }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        "https://c5zaskxsitwlc33abxxgi3smli0lydfl.lambda-url.us-east-1.on.aws/api/service-enquiry",
        {
          name,
          email,
