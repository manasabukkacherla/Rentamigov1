...
const SearchBar: React.FC<SearchBarProps> = ({ searchQuery, setSearchQuery, selectedTags, setSelectedTags }) => {
          ...
          {selectedTags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedTags.map((tag) => (
                ...
                >
                  {tag}
                  <button onClick={() => removeTag(tag)} className="ml-1 text-black hover:text-gray-700">
                    <X className="h-4 w-4" />
                  </button>
                </span>
              ))}
