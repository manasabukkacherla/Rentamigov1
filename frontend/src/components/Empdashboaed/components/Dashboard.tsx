...
import { Building2, Home, Store, DollarSign, Clock, Phone, Calendar, ArrowRight, User, Briefcase } from 'lucide-react';
...
const LeadStatusBadge: React.FC<{ status: Lead['status'] }> = ({ status }) => {
  ...
  const statusClasses = {
    ...
    'viewing-scheduled': "bg-gray-300 text-black dark:bg-gray-600 dark:text-white",
...
const Dashboard: React.FC<DashboardProps> = ({ stats }) => {
  ...
  const groupWidth = (barWidth * totalBarsInGroup) + (barGap * (totalBarsInGroup - 1));
  ...
  return (
    <div className="p-8">
      ...
      {/* Stats Cards */}
      <h2 className="text-3xl font-light mb-8 text-black dark:text-white tracking-tight">Property Overview</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { icon: Building2, label: 'Total Properties', value: stats.totalProperties },
          { icon: Home, label: 'Active Properties', value: stats.activeProperties },
          { icon: Store, label: 'Rented Properties', value: stats.rentedProperties },
          { icon: DollarSign, label: 'Monthly Revenue', value: `$${stats.monthlyRevenue.toLocaleString()}` }
        ].map(({ icon: Icon, label, value }) => (
          <div key={label} className="bg-white dark:bg-black p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-xl transition-all duration-300">
            <div className="flex items-center space-x-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-900">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold text-black dark:text-white mt-1">{value}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-xl transition-all duration-300">
          <h3 className="text-lg font-medium mb-6 text-black dark:text-white">Property Distribution</h3>
        ...
        <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-medium text-black dark:text-white">Revenue Trends</h3>
            <div className="flex items-center space-x-4">
          ...
          <div className="relative h-80">
            ...
            <div className="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-xs text-gray-600 dark:text-gray-400">
              {[maxValue, maxValue * 0.75, maxValue * 0.5, maxValue * 0.25, 0].map((value) => (
                <span key={value} className="transform -translate-y-1/2 w-16 text-right pr-4">
            ...
            <div className="ml-20 h-full relative pb-8">
              ...
              <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
                {stats.revenueHistory.map((item, groupIndex) => {
                  const groupX = (groupIndex * (groupWidth + groupGap)) + (chartWidth - stats.revenueHistory.length * (groupWidth + groupGap)) / 2;
                  return (
                    <g key={groupIndex}>
                      {/* Rental bar */}
                      <rect
                ...
                {stats.revenueHistory.map((item, index) => (
                  <div key={index} className="text-center">
                    ...
                    <div className="text-xs font-medium text-black dark:text-white mt-1">
                      {formatCurrency(item.rental + item.maintenance + item.deposits)}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-white dark:bg-black p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-lg hover:shadow-xl transition-all duration-300 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-medium text-black dark:text-white">Active Leads</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-800">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600 dark:text-gray-400">Property</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {stats.leads.map((lead) => (
                  <tr key={lead.id} className="group hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                    <td className="py-3 px-4">
                      <div>
                        <div className="text-sm font-medium text-black dark:text-white">{lead.name}</div>
                        <div className="text-xs text-gray-600 dark:text-gray-400">{lead.email}</div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-black dark:text-white">{lead.propertyInterest}</div>
                    </td>
                    <td className="py-3 px-4">
