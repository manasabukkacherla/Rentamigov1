...
import type { BlogPostType, Comment as ImportedComment, Review as ImportedReview, Review } from "../Blogs/types/type"
...
const BlogDetail: React.FC = () => {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
      ...
      if (post && newComment.trim()) {
        const newCommentObj: Comment = {
          _id: Date.now().toString(), // ✅ Unique ID to avoid conflicts
          comment: newComment,
          author: {
            fullName: "You",
            avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlcnxlbnwwfHx8fHw%3D&auto=format&fit=crop&w=1760&q=80",
            _id: "",
            name: undefined
          },
  ...
  const handleDeleteComment = (commentId: string) => {
    if (post) {
      const updatedCommentsList = post.comments.filter((comment) => comment._id !== commentId);
  
      setPost({
        ...post,
        comments: updatedCommentsList, // ✅ Update comment count
      });
    }
  };
