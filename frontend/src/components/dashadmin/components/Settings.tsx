...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    setErrors(newErrors);
  }, [userData.currentPassword, userData.newPassword, userData.confirmPassword]);
  ...
  const handlePasswordChange = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/api/sign/user/update-password/${userData.id}`,
        {
      ...
      const errorMessage = error.response?.data?.error || 'Error updating password. Please try again.';
          ...
          <div className="relative">
            ...
            <div className="relative">
              <input 
                ...
                onChange={(e) => setUserData(prevState => ({ ...prevState, currentPassword: e.target.value }))}
              />
              <button
                type="button"
