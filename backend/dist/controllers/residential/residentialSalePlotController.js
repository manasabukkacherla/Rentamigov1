...
exports.deletePlotById = exports.updatePlotById = exports.getPlotById = exports.getAllPlots = exports.createPlot = void 0;
const salePlot_1 = __importDefault(require("../../models/residential/salePlot"));
const _ = require("lodash");
const generatePropertyId = async () => {
    try {
        const prefix = "RA-RESSEPL";
        const highestPlot = await salePlot_1.default.findOne({
        ...
        const existingWithExactId = await salePlot_1.default.findOne({ propertyId });
        if (existingWithExactId) {
            console.log(`Property ID ${propertyId} already exists, trying next number`);
            const forcedNextNumber = nextNumber + 1;
            const forcedPropertyId = `${prefix}${forcedNextNumber.toString().padStart(4, '0')}`;
            const forcedExisting = await salePlot_1.default.findOne({ propertyId: forcedPropertyId });
            if (forcedExisting) {
...
const createPlot = async (req, res) => {
    try {
        ...
        const newLeasePlot = new salePlot_1.default(plotData);
        const savedLeasePlot = await newLeasePlot.save();
        // Return populated plot data with user information
        const populatedLeasePlot = await salePlot_1.default.findById(savedLeasePlot._id)
...
const getAllPlots = async (req, res) => {
    try {
        const leasePlots = await salePlot_1.default.find()
            .populate('metadata.createdBy', 'name email')
            .select('-__v')
...
const getPlotById = async (req, res) => {
    try {
        const leasePlot = await salePlot_1.default.findOne({ propertyId: req.params.propertyId })
            .populate('metadata.createdBy', 'name email')
            .select('-__v');
        if (!leasePlot) {
...
const updatePlotById = async (req, res) => {
    try {
        ...
        const existingDoc = await salePlot_1.default.findById(documentId);
        if (!existingDoc) {
            return res.status(404).json({
                success: false,
        ...
        const updatedDoc = await salePlot_1.default.findByIdAndUpdate(documentId, { $set: mergedData }, { new: true, runValidators: true });
        res.status(200).json({
            success: true,
...
const deletePlotById = async (req, res) => {
    try {
        const data = await salePlot_1.default.findByIdAndDelete(req.params.id);
        if (!data) {
            return res.status(404).json({
                success: false,
