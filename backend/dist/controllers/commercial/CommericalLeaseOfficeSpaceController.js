...
exports.deleteLeaseOfficeSpace = exports.updateLeaseOfficeSpace = exports.getLeaseOfficeSpaceById = exports.getLeaseOfficeSpaces = exports.createLeaseOfficeSpace = void 0;
const CommercialLeaseOfficeSpace_1 = __importDefault(require("../../models/commercial/CommercialLeaseOfficeSpace"));
const lodash_1 = __importDefault(require("lodash")); // install via: npm i lodash
...
const generatePropertyId = async () => {
    try {
        ...
        const highestOfficeSpace = await CommercialLeaseOfficeSpace_1.default.findOne({
        ...
        const existingWithExactId = await CommercialLeaseOfficeSpace_1.default.findOne({ propertyId });
        if (existingWithExactId) {
            // In case of collision, recursively try the next number
            console.log(`Property ID ${propertyId} already exists, trying next number`);
            const forcedNextNumber = nextNumber + 1;
            const forcedPropertyId = `${prefix}${forcedNextNumber.toString().padStart(4, '0')}`;
            const forcedExisting = await CommercialLeaseOfficeSpace_1.default.findOne({ propertyId: forcedPropertyId });
            if (forcedExisting) {
...
const createLeaseOfficeSpace = async (req, res) => {
    try {
        ...
        if (formData.officeSpaceDetails?.cabins === 'Available') {
            cabinsDetails = {
                ...
                sizes: typeof formData.officeSpaceDetails?.cabinsDetails?.sizes === 'string'
                    ? formData.officeSpaceDetails?.cabinsDetails?.sizes.split(',').map((s) => s.trim())
        ...
        const leaseOfficeSpace = new CommercialLeaseOfficeSpace_1.default(leaseOfficeSpaceData);
...
exports.createLeaseOfficeSpace = createLeaseOfficeSpace;
// Get all lease office space listings
const getLeaseOfficeSpaces = async (req, res) => {
    try {
        const leaseOfficeSpaces = await CommercialLeaseOfficeSpace_1.default.find({}).sort({ 'metadata.createdAt': -1 });
        res.status(200).json({
            success: true,
...
exports.getLeaseOfficeSpaces = getLeaseOfficeSpaces;
// Get single lease office space listing
const getLeaseOfficeSpaceById = async (req, res) => {
    try {
        const propertyId = req.params.propertyId;
        const leaseOfficeSpace = await CommercialLeaseOfficeSpace_1.default.findOne({ propertyId });
        if (!leaseOfficeSpace) {
...
exports.getLeaseOfficeSpaceById = getLeaseOfficeSpaceById;
const updateLeaseOfficeSpace = async (req, res) => {
    try {
        ...
        const existingDoc = await CommercialLeaseOfficeSpace_1.default.findById(documentId);
        if (!existingDoc) {
            return res.status(404).json({
                success: false,
        ...
        const updatedDoc = await CommercialLeaseOfficeSpace_1.default.findByIdAndUpdate(documentId, { $set: mergedData }, { new: true, runValidators: true });
        res.status(200).json({
            success: true,
...
exports.updateLeaseOfficeSpace = updateLeaseOfficeSpace;
// Delete a lease office space listing
const deleteLeaseOfficeSpace = async (req, res) => {
    try {
        const leaseOfficeSpace = await CommercialLeaseOfficeSpace_1.default.findByIdAndDelete(req.params.id);
        if (!leaseOfficeSpace) {
            return res.status(404).json({
                success: false,
                message: 'Lease office space listing not found'
...
exports.deleteLeaseOfficeSpace = deleteLeaseOfficeSpace;
