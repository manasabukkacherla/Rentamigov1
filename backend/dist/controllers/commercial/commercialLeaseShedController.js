...
exports.deleteShed = exports.updateShed = exports.getShedById = exports.getSheds = exports.createShed = void 0;
const CommercialLeaseShed_1 = require("../../models/commercial/CommercialLeaseShed");
const lodash_1 = __importDefault(require("lodash"));
const generatePropertyId = async () => {
    try {
        const prefix = "RA-COMLESD";
        const highestShed = await CommercialLeaseShed_1.CommercialLeaseShed.findOne({
        ...
        const existingWithExactId = await CommercialLeaseShed_1.CommercialLeaseShed.findOne({ propertyId });
        if (existingWithExactId) {
            console.log(`Property ID ${propertyId} already exists, trying next number`);
            const forcedNextNumber = nextNumber + 1;
            const forcedPropertyId = `${prefix}${forcedNextNumber.toString().padStart(4, '0')}`;
            const forcedExisting = await CommercialLeaseShed_1.CommercialLeaseShed.findOne({ propertyId: forcedPropertyId });
            if (forcedExisting) {
...
const createShed = async (req, res) => {
    try {
        ...
        const shed = new CommercialLeaseShed_1.CommercialLeaseShed(shedData);
        await shed.save();
        res.status(201).json({
            success: true,
...
const getSheds = async (req, res) => {
    try {
        const sheds = await CommercialLeaseShed_1.CommercialLeaseShed.find();
        res.status(200).json({
            success: true,
            count: sheds.length,
            data: sheds
...
const getShedById = async (req, res) => {
    try {
        ...
        const shed = await CommercialLeaseShed_1.CommercialLeaseShed.findOne({ propertyId });
        if (!shed) {
...
const updateShed = async (req, res) => {
    try {
        ...
        const existingDoc = await CommercialLeaseShed_1.CommercialLeaseShed.findById(documentId);
        if (!existingDoc) {
            return res.status(404).json({
                success: false,
        ...
        const updatedDoc = await CommercialLeaseShed_1.CommercialLeaseShed.findByIdAndUpdate(documentId, { $set: mergedData }, { new: true, runValidators: true });
        res.status(200).json({
            success: true,
...
const deleteShed = async (req, res) => {
    try {
        const data = await CommercialLeaseShed_1.CommercialLeaseShed.findByIdAndDelete(req.params.id);
        if (!data) {
            return res.status(404).json({
                success: false,
