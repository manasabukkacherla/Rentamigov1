...
exports.deleteLeasePlotById = exports.updatePlotById = exports.getLeasePlotById = exports.getAllLeasePlots = exports.createLeasePlot = void 0;
const commercialLeasePlot_1 = __importDefault(require("../../models/commercial/commercialLeasePlot"));
const generatePropertyId = async () => {
    try {
        const prefix = "RA-COMLEPL";
        const highestPlot = await commercialLeasePlot_1.default.findOne({
        ...
        const existingWithExactId = await commercialLeasePlot_1.default.findOne({ propertyId });
        if (existingWithExactId) {
            console.log(`Property ID ${propertyId} already exists, trying next number`);
            const forcedNextNumber = nextNumber + 1;
            const forcedPropertyId = `${prefix}${forcedNextNumber.toString().padStart(4, '0')}`;
            const forcedExisting = await commercialLeasePlot_1.default.findOne({ propertyId: forcedPropertyId });
            if (forcedExisting) {
...
const transformPlotData = (formData) => {
    ...
    const defaultIfEmpty = (value, def) => (value === undefined || value === null || value === '' ? def : value);
    // Lease Details
...
const createLeasePlot = async (req, res) => {
    try {
        ...
        const newLeasePlot = new commercialLeasePlot_1.default(plotData);
        const savedLeasePlot = await newLeasePlot.save();
        // Return populated plot data with user information
        const populatedLeasePlot = await commercialLeasePlot_1.default.findById(savedLeasePlot._id)
...
exports.createLeasePlot = createLeasePlot;
// Get all commercial lease plots
const getAllLeasePlots = async (req, res) => {
    try {
        const leasePlots = await commercialLeasePlot_1.default.find()
            .populate('metadata.createdBy', 'name email')
            .select('-__v')
...
exports.getAllLeasePlots = getAllLeasePlots;
// Get a specific commercial lease plot by ID
const getLeasePlotById = async (req, res) => {
    try {
        const propertyId = req.params.propertyId;
        const leasePlot = await commercialLeasePlot_1.default.findOne({ propertyId })
            .populate('metadata.createdBy', 'name email')
            .select('-__v');
        if (!leasePlot) {
...
exports.getLeasePlotById = getLeasePlotById;
const updatePlotById = async (req, res) => {
    try {
        const propertyId = req.params.id;
        const updateData = req.body;
        const leasePlots = await commercialLeasePlot_1.default.findOneAndUpdate({ propertyId }, { $set: updateData }, { new: true });
        if (!leasePlots) {
            return res.status(404).json({
                success: false,
...
const deleteLeasePlotById = async (req, res) => {
    try {
        const propertyId = req.params.id;
        const leasePlots = await commercialLeasePlot_1.default.findOneAndDelete({ propertyId });
        if (!leasePlots) {
            return res.status(404).json({
                success: false,
                error: 'lease plot property not found'
...
exports.deleteLeasePlotById = deleteLeasePlotById;
