...
export const normalizeProperty = (item: any) => ({
  ...
  title:
    ...
    'Untitled Property',
  location:
    `${item.basicInformation?.address?.city || ''}, ${item.basicInformation?.address?.state || ''}` ||
  ...
  price:
    ...
    0,
  area:
    item.propertyDetails?.area?.totalArea ||
    item.propertySize ||
    ...
    0,
  image:
    item.media?.photos?.exterior?.[0] ||
    item.media?.photos?.plot?.[0] ||
    'https://via.placeholder.com/400x300?text=No+Image',
  postedDate:
    item.metadata?.createdAt instanceof Date
      ? item.metadata.createdAt.toISOString().slice(0, 10)
      : '',
  status: (item.metadata?.status || item.metaData?.status || 'active').toLowerCase() as
    | 'active'
    | 'pending'
    | 'resolved',
