...
//     flooring: { type: String, required: false },
...
interface IResidentialRentIndependentHouse extends Document {
  ...
  basicInformation: {
    ...
    address: {
        ...
        state: string;
        zipCode: string;
        pinCode: string; // This was missing in schema
        location: {
          latitude: string; // Frontend uses string, not number
          longitude: string; // Frontend uses string, not number
        };
      };
  };
  propertySize: number;
...
const residentialRentIndependentHouseSchema = new mongoose.Schema<IResidentialRentIndependentHouse>({
  propertyId: {
    type: String,
    required: false,
    unique: true, // Make this unique to avoid duplicates
  },
  basicInformation: {
    title: { type: String, required: false }, // Changed to false for edit to work
    address: {
      ...
      pinCode: { type: String, required: false }, // ADDED THIS MISSING FIELD
      location: {
        latitude: { type: String, required: false }, // Changed to String to match frontend
        longitude: { type: String, required: false }, // Changed to String to match frontend
      },
    },
  },
  propertySize: { type: Number, required: false },
  ...
  timestamps: true // Enable Mongoose timestamps to automatically handle createdAt and updatedAt
});

// Add index for better performance
residentialRentIndependentHouseSchema.index({ propertyId: 1 });
residentialRentIndependentHouseSchema.index({ 'metadata.createdBy': 1 });
