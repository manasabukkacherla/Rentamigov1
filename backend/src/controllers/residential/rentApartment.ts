...
import ResidentialRentAppartment from '../../models/residential/residentialRentAppartment';

const generatePropertyId = async (): Promise<string> => {
    try {
        const prefix = "RA-RESREAP";

        const highestShowroom = await ResidentialRentAppartment.findOne({
        ...
        const existingWithExactId = await ResidentialRentAppartment.findOne({ propertyId });
        ...
        if (existingWithExactId) {
            ...
            const forcedExisting = await ResidentialRentAppartment.findOne({ propertyId: forcedPropertyId });
...
export const createRentApartment = async (req: Request, res: Response) => {
  try {
    ...
    const apartment = new ResidentialRentAppartment(apartmentData);
...
export const getAllRentApartments = async (req: Request, res: Response) => {
  try {
    ...
    const apartments = await ResidentialRentAppartment.find(filters)
      .skip(skip)
      .limit(limit)
      .sort({ 'metadata.createdAt': -1 });

    const total = await ResidentialRentAppartment.countDocuments(filters);
...
export const getRentApartmentById = async (req: Request, res: Response) => {
  try {
    const apartment = await ResidentialRentAppartment.findById(req.params.id);
...
export const updateRentApartment = async (req: Request, res: Response) => {
  try {
    ...
    const updatedApartment = await ResidentialRentAppartment.findByIdAndUpdate(
      req.params.id,
      {
        ...req.body,
        metadata: {
...
export const getUserApartments = async (req: Request, res: Response) => {
  try {
    ...
    const apartments = await ResidentialRentAppartment.find({
    ...
    const total = await ResidentialRentAppartment.countDocuments({
      'metadata.createdBy': req.user?._id
    });
