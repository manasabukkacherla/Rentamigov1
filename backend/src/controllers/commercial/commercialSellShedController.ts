...
export const createCommercialShed = async (req: Request, res: Response) => {
  try {
    ...
    // FIX: Use metadata (lowercase 'd') to match schema
    shedData.metadata = {
      createdBy: shedData.metadata?.createdBy, 
      createdAt: new Date(),
...
export const getAllCommercialSheds = async (req: Request, res: Response) => {
  try {
    ...
    if (city) {
      query['basicInformation.address.city'] = { $regex: city, $options: 'i' };
    }
    ...
    // Execute query with projection for list view
    const sheds = await CommercialShed.find(query)
      .select('propertyId basicInformation shedDetails.totalArea pricingDetails.propertyPrice media.photos.exterior metadata')
      .sort(sortQuery)
      .skip(skip)
      .limit(limitNum)
      .lean();
...
export const updateCommercialShed = async (req: Request, res: Response) => {
  try {
    ...
    const existingDoc = await CommercialShed.findOne({ propertyId });
    if (!existingDoc) {
      return res.status(404).json({
        success: false,
        message: 'Commercial shed listing not found'
      });
    }

    // ✅ IMPROVED: Handle media properly - detect base64 vs URLs
    if (incomingData.media && incomingData.media.photos) {
      Object.keys(incomingData.media.photos).forEach((category) => {
        incomingData.media.photos[category] = incomingData.media.photos[category].map((item: any) => {
          // If it's a base64 string (starts with data:), keep it for processing
          ...
          return item;
        }).filter((item: any) => item); // Remove null/undefined items
      });
    }

    if (incomingData.media && incomingData.media.videoTour) {
      if (typeof incomingData.media.videoTour !== "string" && incomingData.media.videoTour.url) {
        incomingData.media.videoTour = incomingData.media.videoTour.url;
      }
    }

    if (incomingData.media && incomingData.media.documents) {
      incomingData.media.documents = incomingData.media.documents.map((doc: any) => {
    ...
    // ✅ Now merge safely
    const mergedData = _.merge(existingDoc.toObject(), incomingData);

    const updatedDoc = await CommercialShed.findOneAndUpdate(
      { propertyId },
      { $set: mergedData },
      { new: true, runValidators: true }
    );
    ...
    console.log("Error updating commercial shed:", error);
    return res.status(500).json({
      success: false,
      error: 'Failed to update commercial shed: ' + error.message
    });
  }
};
