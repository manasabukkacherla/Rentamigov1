...
const generatePropertyId = async (): Promise<string> => {
  ...
  const exists = await CommercialLeaseCoveredSpace.findOne({ propertyId: id });
  if (exists) return generatePropertyId(); // recursive if duplicate
...
export const createCommercialLeaseCoveredSpace = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Create failed', message: (error as Error).message });
  }
};
...
export const updateCommercialLeaseCoveredSpace = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const updated = await CommercialLeaseCoveredSpace.findByIdAndUpdate(req.params.id, updatedData, {
      new: true, runValidators: true
    });
